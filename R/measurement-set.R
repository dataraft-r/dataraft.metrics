measure_set <- function(
  ...
  if (
    ...
    if (
      ...
      owned <- optional_lake("dr_connect_lake")(source$lake, read_only = TRUE)
      on.exit(optional_lake("dr_disconnect_lake")(owned), add = TRUE)
      x$output_lake <- owned
    }
  }
  results <- list()
...
report_connection <- function(lake, read_only) {
  ...
  if (inherits(lake, "dr_config")) {
    ...
    return(optional_lake("dr_connect_lake")(lake, read_only = read_only))
  }
  if (is.character(lake) && length(lake) == 1L && !is.na(lake)) {
    ...
    return(optional_lake("dr_open_lake")(lake, read_only = read_only))
  }
  dataraft.core::dr_internal_abort(
    subclass = "dataraft_error_metrics",
